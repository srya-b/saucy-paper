...
In this section we relate it to the UC frameowwor and illustrate how NomosUC can be used to realizes UC.
We solve a key challenge introduced by linear channels on process configurations that can be described, and mitigate it by proposing an abstract channel design, called \emph{providerless channels}.
Providerless channels appeared earlier in the code of \simcom in Figure~\ref{fig:simcom} (lines 4,5), where we create a set of channels for \Fro.
Below, we explain how this mechanism works.
Providerless channels allow UC execution to support an arbitrary number of instances of a protocol or ideal functionality.
We conclude by covering the appropriate composition operators.
...
All linear channels are created when they are offered by a process called the ``provider'', and they are given to a process called the ``client''. 
This parent-child relationship creates a constraint that prevents the process configurations in Figures \ref{fig:pandq} and \ref{fig:threeway}.
If we opt to communicate only over channels offered by these main processes in the configuration, then we are limited to acyclical topologies.
In Figure \ref{fig:pandq} $P$ must already exist to offer its channel to $Q$, but $Q$ must also exist before $P$ to offer its channel to $P$.
Similarly, in the three-way configuration in Figure \ref{fig:threeway}, it is a more naturally occuring situation that suffers from the same problem.
...
Shared channels can be inserted to interrupt cyclical topologies because they can have multiple concurrent clients regardless (i.e. shared) regardless of how they were created.
They allow us to define a message buffer called a communictor, that allows multiple other processes act as its clients and use its channel to send messages.
Any process can attempt to \emph{acquire} its channel through a locking mechanism, check for new messages with the label \mb{poll}, and place another message in the bugger with \m{push}.
...
The construction of a providerless channel between $P$ and $Q$ is shown in Figure~\ref{fig:newpandq}.
Each providerless channel has a communicator process in between its two endpoints, and dummy processes are defined that relay with the communicator and offer the desired session type to $P$ or $Q$.
In the figure, the dummy process \m{a_P} offers session type \m{PtoQ} to $P$ and \m{a_Q} offers the dual of \m{PtoQ}.
The session type \m{PtoQ} offered to $P$ is almost identical to the type offered to $Q$. 
The type offered to $Q$ swaps internal and external choice operations because $P$ and $Q$ are both ``clients'' of linear channels at easch end of the providerless channel.
The only constraint this imposes on the programmer is implementing all ITMs (as processes) as if they are clients to a linear channel.
The dummy processes that offer the linear channels are simple case statemes that translate between the type offered by the communicator (a functional type) to the type offered to either $P$ or $Q$, and vice versa.
Owing to their simplicity, given a specification of the two types and a mapping between them, these process are easily generated at compile-time, and the programmer can ignore them entirely.

%where ther internal and external choice operators are swapped, beacuse \m{a_P} and \m{a_Q} are both providers of the channel to $P$ and $Q$. 
%All internal choice operators correspond to messages from the dummy operator.
%We don't lose expressivity by doing this, but it does require the programmer to select one process to see a type $T$ and the other to see its dual. 
%The dummy programs are simple case statements that translate between the type offered by the communicator to the type that they offer to the actual process (the desired session type).
%Owing to their simplicity, given a specification of the two types and a mapping between them, these process are easily generated at compile-time, and the programmer can ignore them entirely.

Providerless channels are easy to spawn, all the processes involved are simple. 
We provide an insight into how providerless channels are implemented by showing how to realize the topology in Figure \ref{fig:threeway}.
Canonically, we define one process, in this case $M$ to be the first process spawned is responsible for creating the rest of the topology.
...
\end{lstlisting}
The first providerless channel call is syntax sugar for the following code. We use diffeent channel nemaes to make it clear which channels they are.
\begin{lstlisting}[basicstyle=\scriptsize\BeraMonottFamily, mathescape, frame=single, numbers=left, xleftmargin=2em, xrightmargin=0.9em]
#c $\leftarrow$ communicator_p2q $\leftarrow$ k rng
$\$$forq $\leftarrow$ convert_p2q $\leftarrow$ #c
$\$$forp $\leftarrow$ convert_q2p $\leftarrow$ #c
\end{lstlisting}
The processes $P$ spawned with these channels is typed as follows.
\begin{lstlisting}[basicstyle=\scriptsize\BeraMonottFamily, mathescape, frame=single, numbers=left, xleftmargin=2em, xrightmargin=0.9em]
$\nproc$ P{n}: (k: Int), (rng: [Bit]), (sid: SID\_P), ($\$$p2m: pandm),
  ($\$$p2a: pandq{n}) |- ($\$$ch: 1) =
{
  $\nmatch$ $\$$p2m, $\$$p2q (
...
\begin{figure}
    ...
    \begin{subfigure}{0.3\textwidth}
    \begin{center}
    \includegraphics[scale=0.5]{figures/threeway.png}
