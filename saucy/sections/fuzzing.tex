UC analysis replaces individual property-based testing with a testing a relationship between a protocol and an idealized version.
...
%We implement a examples that demonstrate how even informal analsysis can be used to analyze protocols across composition and that express properties such as fairness and adversarial influence on distributions--things UC is known to be well-suited to proving.
...
%Finally we conclude that we can reason about liveness by asserting the necessary protocol-specific predicates, or reason about what level of adversarial fairness is required for protcols to make progress/terminate. \todo{the last part here is losing the plot}
...
%
%\plan{Normally, liveness is associated with some notion of adversarial fairness. There is a predicate that says as long as these messages are delivered, then the protocol will do something good. Another version of this is that as long as honest partiesa are activated inifinitely often, something good will happen. What we can do with UC here is that we can test different adverasarial models of fairness to determine under what conditions is fair. What messages, how many, and under what constraints of ordering are required to allow the protocols to termiante or make progress to the next step?}
...
\subsection{Implementing Fuzz Testing}
% describe that we use quickcheck for fuzzing and what quickcheck is
% what kinds of randomness can we generate and to do what?
% adv scheduling, varying party input, choosin crupt input, censoring/filtering
...
% generating environments do take into account types of failures we anticipate, generic testing yields a much slower but still successful approach
...
The test enviromments we generate share some commanlity across all of the protocols that we examine.
...
% looking for output distributions, one can program statistical methods for defining a particular assertion
% or simply collect relevant output information across n test cases and manually inspect
\begin{center}
...
\subsection{Testing Asynchronous Protocols}
...
% manually testing these things is fine anyone can do that, can we relegate this to fuzz testing as well making it easy to analyze and detect problems in? 
