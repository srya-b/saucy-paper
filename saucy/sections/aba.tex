...
In this section, we set out to examine whether fuzz testing can be a useful tool in UC to detect implementation-level bugs.
...
In our setup, we design the generators and choose the bugs that we inject into our protocols, and we have to be careful to design the generators to target specific states in the code where we think failures might occur rather than targetting them specifically at the bugs that we inject.
...
%False positives encountered in our analysis of liveness in the next section are not discarded.
...
%The protocols should be probabilistic and use randomness in different ways.
...
%The BenOr protocol makes use of local randomness. 
...
\end{itemize}
All three of these properties can be captured by the ideal functionality for binary agreement in Figure~\ref{fig:faba}.% but we examine checking failures of the \emph{Termination} property in the next section.
% We rely on our asynchronous model to capture the \emph{eventual delivery} guarantee that underpins the \emph{Termination} property.
