...
$\nproc$ f_multisession_p2f_input[K][K1]{p2fn,f2pn} : 
  $\tg{(* default args *)}$
  (#p2f: comm[p2fms[p2ms]][K]), 
  (#f2p: comm[f2pms[ms2p]][K]), 
  (#a2f: comm[a2fms[a2ms]][K]), 
  (#f2a: comm[f2ams[ms2a]][K]) $\vdash$ ($\$$ch: 1) =
{
  ...
  msg = $\nrecv$ #p2f ;
  $\ncase$ msg (
    P2F(pid, P2MS(ssid, msg)) =>
      ...
      $\nthen$
      	#new_p2ssid $\leftarrow$ channel_init[K1][p2f] $\leftarrow$ ;
      	#new_ssid2p $\leftarrow$ channel_init[K1][f2p] $\leftarrow$ ;
      	#new_a2ssid $\leftarrow$ channel_init[K1][a2f] $\leftarrow$ ;
      	#new_ssid2a $\leftarrow$ channel_init[K1][f2a] $\leftarrow$ ;
      
      	$\$$p2ssidnew $\leftarrow$ append $\$$p2ssid #new_p2ssid ;
      	$\$$ssid2pnew $\leftarrow$ append $\$$ssid2p #new_ssid2p ;
      	$\$$a2ssidnew $\leftarrow$ append $\$$a2ssid #new_a2ssid ;
      	$\$$ssid2anew $\leftarrow$ append $\$$ssid2a #new_ssid2a ;
      
      	$\$$chprime $\leftarrow$ f_wrapper[K1][f2p,p2f][f2a,a2f] $\leftarrow$
               k rng sid clist $\#$p2ssid $\#$ssid2p 
               $\#$a2ssid $\#$ssid2a $\#$z ;
      $\nend$
  
      #ch $\leftarrow$ get_channel #p2ssid ssid ;
      $\nwithdraw$ K K1 {p2fn} ;
      $\$$ch.SEND ;
      $\nsend$ $\$$ch P2F(pid, msg) ;
      $\npay$ $\$$ch {p2fn : K1} ;
  	
    $\$$ch $\leftarrow$ f_multisession_f2p_i[K][K1] $\leftarrow$ $\tg{(* args *)}$ 	
      $\$$p2ssidnew $\$$ssid2pnew $\$$a2ssidnew $\$$ssid2anew 0;	
}
\end{lstlisting}
...
\todo{this is copied from the main body and still references the appendix, make this appenfix referenced.}
...
The multisession extension is mentioned in greater detail in Appendix~\ref{app:ms}.
...
We elaborate on the remaining theorems in this section in the Appendix.
