...
The ITM computational model is extremely flexible to capture as universal a setting as possible.
...
A process's channel is created when it is spawned and can not be given to another process except the calling process.
Therefore, the figure illustrates a scenario where they each must spawn each other, which is impossible.
...
Communicators offer channels with a shared session type and acts as a buffer between two processes.
