...
Therefore, \m{execUC} creates only the shared parts of the providerless channel and passes them as input to the wrapped processes. For example \Z and \A are connected by the following channels:
%\begin{figure*}
...
Its design of encapsulating the protocol parties forces the wrapper round \F to do the same to de-multiplex inputs from \partywrapper.
...
\begin{mathpar}
	...
	\inferrule*[right=emulate]
	{
		...
		\forall \langle \A \leftrightarrow \pi \rangle \ | \ \Delta_4[\Tokentypes][\mathrm{T}_{\A}] \vdash \A :: \Delta_3',\\
