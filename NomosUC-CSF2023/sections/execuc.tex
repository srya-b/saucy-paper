...
Below, we introduce how the UC experiment is implemented in NomosUC, and then provide a composition operator $\circ$ for protocols and simulators that satisfies Theorem~\ref{thm:singlecomp}.
...
The type definition of \inline{execUC} is straightforward \todo{once we have clarity on the intro and type system size we can include the typedef of here}. 
It takes virtual tokens types and message types, between all the main processes, as type parameters, and it takes a security parameter $k$ and random bit string $r$ as the only functional parameters.
...
\end{lstlisting}
where the type of \inline{#z_to_a} is given in Section~\ref{subsec:communicators} and its type parameters like $\tp{z2a}$ and $\tp{z2an}$ (the import amount) are the type parameters of \inline{execUC}.
When processes are spawned, their shell codes correctly connect the channel endpoints internally.

The processes definitions of \Z, \A, \F, and $\Pi$ are not passed as parameters to \inline{execUC} because NomosUC currently doesn't support passing them as parameters. Therefore, \inline{execUC} is defint in terms of a imported user module \inline{PS}.
...
The environment \Z is the first main processes because it determines the list of corrupt parties \inline{clist : [PID]} and the SID (a user-defined type) for the execution. \Z offers a channel of type \msf{EtoZ} defined as
...
After the corruptions and SID, the type concludes when \Z outputs a bit as its guess of which world it's in. 
We later define indistiguishability between the two worlds over the distribution of output bits being computationally indistinguishable from coin flipping.
...
The core of the UC framework is the ideal functionality: it captures all of the desired security properties and the protocol interface in a simple trusted third party. 
Its session types with parties contrains message ordering and computational resources, and it provides the user insight into the functionality and its interface.
We believe these insights aid in writing software that can treat NomosUC definitions in a black-box and modular way---a critial advantage of the UC framework.
Fitting session types to ideal functionalities requires some tradeoffs with the limitations of session types, and we use the 2-way authenticated channel functionality, \Fauth, to highlight them. 

\Fauth accepts messages from one of two parties and allows \A to decide when the message is delivered to the other.
At a given moment, party $P_1$ can give input to, or expect output from, \Fauth at a given moment
From our discussion motivating providerless channels, we note that this is a communication pattern that a single session type cannot capture.
A party using \Fauth can both send a message and receive a message at any given time.
...
\end{tabbing}}
\end{center}
We can mitigate such tradeoffs by making use of polymorphism in our types. In this example the session types are parameterized by a type \inline{a}. 
The functionality doesn't know the type and, therfore, the type system would make it impossible for \Fauth to send something of type \inline{a} to a receiver before receiving input of type \inline{a} from a sender. 
In general, we can use this trick anywhere we split communication into two-unidirectional channels to enforce ordering of messages between channels. 

This tradeoff also comes up in some functionalities that do not exhibit this communication pattern, but handle a dynamic number of parties without a communication pattern like \Fro described in a later section. 
...
Its purpose is to simplify the UC experiment construction, because dynamic party generation requires a process to spawn and connect new channels to them. 
...
It is important to note that like most providerless channels, the construction of channels here is protocols/functionality dependent. The reason is that parts of the channels are converting functionally types messages and forwarding them correctly along a session typed channel.
Typically, such protocol-specific constructions are written by users writing functionalities, but in NomosUC the constructions are so trivial/straightforward that it is possible so systematically generate them based only on the types and minimal user-specification. 
...
The \partywrapper construction affects the import sent from \Z to a protocol party.
Instead of the session type of each party requiring different import from \Z, the communicator forces a constant number sent with each message.
The linear channels that make up the providerless channels, and connect to the parties, enforce dinstinct import requirements.
...
Instead of losing the soul of session types that we're pursuing, we replicate the virtualization of the \partywrapper to meaningfully use session types. 
NomosUC runs \F in a shell and communcates with \A and \partywrapper through providerless channels. The same reasoning about the impact of import in the \partywrapper applies here.
...
The term allows us to quantify statements over only adversaries and environments whose types match, for the protocol/functionality in question, when reasoning about emulation. 
...
Given two protocols $(\pi, \F_1)$, $(\phi, \F_2)$, which we refer to only by \PI and $\phi$, that emulated each other, then $\forall \A$ well-matched with \PI, there $\exists \Sim$ well-matched with $\phi$ s.t. $\forall \Z$ : $\msf{execUC}(\pi, \F_1, \Z, \A)$ $\approx$ $\msf{execUC}(\phi, \F_2, \Z, \Sim)$:
...
An important validation of our approach is the the Dummy Lemma~\ref{thm:dummythicclemma} which shows that a simulator for the dummy adversary satisfies the emulation definition for all adversaries. 
...
%We these extensions and operators validated in NomosUC, we can finally conclude with full composition in Theorem~\ref{thm:composition}.
%The proof follows directly from Theorems~\ref{thm:singlecomp}, \ref{thm:functor}, and a squash protocol described in Appendix~\ref{app:ms}.
