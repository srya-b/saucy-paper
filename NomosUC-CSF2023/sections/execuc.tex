...
An limitation of binary session types is the communication topologies that they can realize.
...
The resulting construction of a providerless channel between $P$ and $Q$ is shown in Figure~\ref{fig:newpandq}.
Each providerless channel has a communicator process in between them, and dummy processes are defined an spawnes that communicate with the communicator and offer the desired session type fo $P$ or $Q$.
In the figure, the dummy process \m{a_P} offers session type \m{PtoQ} to $P$ and \m{a_Q} offers the dual of \m{PtoQ} where ther internal and external choice operators are swapped, beacuse \m{a_P} and \m{a_Q} are both providers of the channel to $P$ and $Q$. 
All internal choice operators correspond to messages from the dummy operator.
We don't lose expressivity by doing this, but it does require the programmer to arbitrarily select one process to see a type $T$ and the other to see its dual. 
...
Using communicators imposes constraints on the amount of import that can be sent over a providerless channel.
The communicator type allows only a constant amount of import to be sent with every message. In our channels, each direction of communication carries its own constant amount of import.
...
One of our main contributions over previous work performing execution cost analysis while allowing dynamic creation of protocol parties and ideal functionalities.
The key design challenge in realizing this feature is resolved by providerless channels.
We introduce a new construct, the \partywrapper \MX, which marshals the protocol parties in a particular execuition.
When another main party, say \Z, sends a message to a particular party with \m{pid}, it sends a generic message to \MX through type:
\begin{tabbing}
    $\mi{type} \; \m{P2F[\tau]\{n\}} = \ichoice{\textcolor{red}{\paypot{n}} \; \mb{p2f}: \m{PID} \arrow \m{\tau} \arrow \m{P2F[\tau]\{n\}}}$
\end{tabbing}
The \partywrapper reads the message, creates an instance of the party with \m{pid} if it doesn't already exist, and spawns it using providerless channels.
Although the generic type \m{P2F} above doesn't express any specific message ordering, the providerless channel that connects \MX, internally, to party \m{pid}, uses the session type of the protocol party.
This ensures that the type system still enforces the protocol ordering required. 
...
After spawning the protocol party with its channels, \MX first must withraw virtual tokens in the amount specified by the sender session type, and then send the messae with the virtual tokens along the channel.

Its communication with \A and \F is similarly handled.
The generic communication types, \m{P2Z} and \m{Z2P}, also imply that there is some constant amount of import sent with the single label in the type, \mb{p2f} and \mb{f2p} respetively.
...
This is not a retriction as emulation and indistinguishabiliy only care that the amounts sent/received by \Z are the same in both worlds.
In general, we're not concerned with reasoning about efficiency or minimizin import requirements for a protocol in UC.
...
$\nproc$ compose[K,K1] 
  ...
  ($\$$z2ro: z2roS), ($\$$ro2z: ro2zS), ($\$$pi2f: pi2fS), 
  ($\$$f2pi: f2piS) |- ($\$$cmp: 1) = {
  $\$$r2p4r, $\$$r2p4p <- channel_init[ro2fS][r2fF]{r2fn}
  $\$$p2r4r, $\$$p2r4r <- channel_init[f2roS][f2rF]{f2rn}
  $\$$ro <- rho[K] <- k rng sid pid $\$$z2ro $\$$ro2z 
    $\$$r2p4r $\$$p2r4r 
  $\$$pi <- pi[K] <- k rng sid pid $\$$r2p4p $\$$p2r4p 
    $\$$pi2f $\$$f2pi
}
\end{lstlisting}
Like any other protocol party, the composition operator accepts linear channels of the required type.
It's channels from \Z are the same type as $\rho$'s communication with \Z, and its channel with the ideal functionality is typed the same as the protocol $\pi$'s session type communicating with its hybrid functionality.
