...
To add to the database in Line 4, the party sends the label \mb{store} over \ic{p2f} and uses \isend to send the message contents in the type: a PID and a key-value pair to store.
...
The session type progresses to the external choice, and \Fdb sends an \mb{Ok} ackknowledgement back to the party (line 7) with \inline{pid}.
The channel recurses back to type \m{db[k][v]} and the process calls itself again with a new list \inline{$\$$tb'} and waits for messages.
...
Canonically in NomosUC, functionalities and parties are given two channes, here \ic{p2f} and \ic{f2p}, to allow unidirectional communication over each if desired or necessary. 
...
We learn that \A has access to the same interface as protocol parties, and, importantly, that \Fdb doesn't leak any information to it~\footnote{The same type means the same code for handling messages on \ic{a2f} so we exclude it to save space}.
