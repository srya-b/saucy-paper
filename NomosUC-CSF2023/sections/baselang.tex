...
Using the communicator type severaly restricts all channels to the same session type \m{comm}. 
...
Take, for example, Figure~\ref{newpandq}. Here we wish to realize communication with the session types mentioned previously: \m{PtoQ} and \m{QtoP}.
...
An important note to providerless channels are that processes $a_p$ and $b_p$ are specific to the session type in question.
These processes map functional messages from the communicator to the linear channel they offer.  
...
Finally we point out that the shared portion of providerless channels, the communicator parts that deals with real tokens, requires a constant, possibly non-zero, amount of import sent over it with each messages. 
In the worst case, this means some message are sent with more import than the underlying session type of the wrapped process needs. 
