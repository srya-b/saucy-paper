...
Regardless of what import is required by the channel offered by, say, $\m{a_P}$, the commuicator \m{c} connecting it to $\m{a_Q}$ must accept and send the maximum import required by any type originating from $\m{PtoQ}$.
...
In the worst case, this means some messages are sent with more import than the underlying session type of $\m{a_i}$ and $\m{b_i}$ requires.
