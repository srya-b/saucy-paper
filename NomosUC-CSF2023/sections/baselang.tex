...
We note that some of the base system rules carry over from existing work by Das et al.~\cite{rast}, and we add to them new features like the write token and import token typing rules in the following section. 
...
The \Fcom process from Figure~\ref{fig:fcomideal} offers an unused channel, but communicates
only over its linear antecedents: the channels to the sender and receiver. 
In general througout this work we don't use the offered channels of processes 
aside from the \emph{providerless channels} which we disuss in detail later in this
section. 
...
send a commitment to \Fcom. The token is tranferred to \Fcom which then 
...
aling $c$ and continues as a $P$ substituting $c$ for a new channel $c'$. Creating a new
channel $c'$ ensures that messages arrive in order. 
Naturally, a message send doesn't consume work but, as we'll see later, can 
...
sent to the chanel provider, the receiver in this case, along with the label.
...
For example, imagine a simple UC executin with an environment \Z, two parties $P_1$ and $P_2$, and some ideal functionality \F.
Normally, we would like a channel between \Z and each of $P_1$/$P_2$, and a channel between \F and each of $P_1$/$P_2$. 
...
The acquire-release paradigm, along with the wirte token, is necessary for shared session types to prevent non-determinism 
Communicators are written to be a sender that acquires the channel and $\mb{push}$es a functionally 
...
We peovide the full typing rule of shared session types in Appendix~\ref{app:typing_rules}.
...
For exampe, in place of channels typed with \m{receiver} or \m{sender} from the commitment example communicator channels would be typed \m{comm}[\m{sender2f}] where 
...
We overcome this constraint by abstracting channels in NomosUC into higher-level consructs called \emph{providerless channels}.
