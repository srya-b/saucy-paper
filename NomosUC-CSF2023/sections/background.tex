...
The transitive relation in Theorem~\ref{thm:singlecomp} allows for replacement of a single instance of $\F_2$ with $\pi$.
UC composition in Theorem~\ref{thm:composition}, which is realized using Theorem~\ref{thm:singlecomp}, allows for replaces an arbitrary number of instances of $\F_2$ running concurrently using the $!$ operator. 
We elide the details of this composition theorem until we have introduced the $!$ operator and it's corresponding composition theorem: 
