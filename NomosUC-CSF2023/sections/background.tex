...
The flexibility of the framework is that \Z gets to determine the exat number of sessions of a protocol running in parallel. The $!\F$ operator is the multisession extension 
of of \F, and is captured by the theorem 
$!\F_1 \xrightarrow{\pi} \F_2$, $!\F_2 \xrightarrow{\phi} \F_3 \implies !\F_1 \xrightarrow{\phi \circ !\pi} F_3$.
This theorem is essential for realizing full UC composition discussed in Section~\ref{sec:execuc}.
...
The ITM model has the advantage that only one process is active at a given time. It results in message scheduling that is essentially deterministic and easily simulated by sequential coputation. 
