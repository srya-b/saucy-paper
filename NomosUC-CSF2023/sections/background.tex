...
Here, \m{execUC} is a UC experiment that takes four parameters and creates the main ITMs of the execution, namely the environment \Z, an adversary \A, and an ideal functionality \F.  
Parties of the protocol $\pi$ are created at runtime when messages are written to them. 
...
\end{equation}
Indistinguishability is denoted by the $\sim$ symbol, and it implies that the ensemble of distributions resulting from the output of \Z in each execution, over all possible security parameters and randomness, have \emph{statistcal difference} that is negligible in $k$. 

For UC realization the protocol in the ideal world is the \idealP, or the \emph{dummy protocol}, that forwads messages between $\F_2$, \Z, and \A. 
Similarly, the real world can also contain an ideal functionality $\F_1$ that may captures network assumptions or assumed primitives the protocol relies on. We call this world the $\F_1$-hybrid world.
