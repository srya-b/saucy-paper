...
Universal Composability (UC) is a framework~\cite{canettiUC} used to define security in cryptographic protocols based on
a real world / ideal world paradigm.
Security is defined as a relation between these two worlds:
the real world features a protocol $\pi$ and an arbitrary adversary \A that controls corrupt parties;
the ideal world comprises of an ideal functionality \F, a trusted third party that carries out the desired task directly.
...
What makes UC the strongest among ideal/real frameworks is that the inputs and number of honest parties are chosen adaptively, and dynamically, by a distinguishing environment \Z, which can communicate with the adversary and interleave its interactions with honest parties.
...
The \msf{execUC} defines the UC experiment in NomosUC that hooks up the channels to the processes in its arguments.
Defining \msf{execUC} in our core calculus, and especially reconciling its the dynamic nature with the static guarantees of our type system will be the central technical challenge we tackle later.
