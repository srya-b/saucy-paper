...
\begin{abstract}
  ...
  short in a few critical ways: namely, capturing an adequate polynomial time notion to express UC in its entirety.
  ...
  introduced by Canetti et al.~\cite{canettiUC}, to ensure polynomial-time computation.
  ...
  in the appendix.
  %and in doing so we can use IST to provide strong
  %static polynomial time guarantees for the generic UC setting rather than limited notions of polynomial 
  %time or constrained versions of UC that prior work realize. Namely, we define a redursive import token
  %definition called the virtual token heirarchy and generic composition operators to reconcile IST with
  %Rast.
  %Unlike prior work, however, we do not provide
  %program logic for proving security or generating proofs of security. We see import session types as a 
  %valuable tool for realizing UC definitions and aiding paper and pencil proofs of UC security.
...
\section{Adding Import to NomosUC} \label{sec:motivate}
\input{sections/motivating}

\input{sections/old_import}

\section{(Unchanged) Base System of Session Types in NomosUC} \label{sec:basic}
