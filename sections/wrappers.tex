...
For example, in the code in Figure~\ref{fig:atomic:real}, $\mathcal{F}_{atomic}$ is telling the wrapper to send it \msf{Broadcast} when it's time to execute it along with arguments required for executing the code.

In addition to scheduling codeblocks with \Wasync, all ITIs that do so must also be spawned with the code to receive activation from the wrapper to execute the scheduled codeblock.
