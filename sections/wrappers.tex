...
Before diving into the details of how the wrappers are implementated, and what consequences appear, we first give an example illustrating the advantage of our model compared to traditional UC communication.
...
Once scheduled, the codeblock is added to the \msf{runqueue} until it is executed by the wrapper (we will elaborate the conditions for executions later).
...
We motivate our relaxing of the balanced environments constraint, first, by addressing Proposition~\ref{prop:balancedenvironments}. 
In summary, the proposition states that when a protocol is extended trivially to instruct $\pi_i$ to send a message to \Adversary proportional to it's import, the environment can ensure the $\pi_i$ always has more import than \Adversary and, therefore, the adversary is no longer simulatable. 
