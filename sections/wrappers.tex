...
In Figure~\ref{fig:asyncwrapper:short}, the async wrapper, \Wasync, accepts two messages from parties and functionalities: leaked messages and code blocks in the form of a \msf{schedule} message (alternatively, when code uses the \textsc{Evetually} keyword).
...
| \; \m{Leak} \text{ of } \m{list[ARGS]}
\end{align*}
The \msf{schedule} messages from prtocol parties are slightly different from those of the funtionality, because they need to include a party identifier whereas there is only \textit{one} ideal functionality in the execution.

On receipt of a \texttt{Schedule(f, args)} message, the codeblock is added to the \msf{runqueue} until it is executed by the wrapper (we will elaborate on the conditions for execution later). 
