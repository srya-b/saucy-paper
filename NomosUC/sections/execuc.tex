...
We first define UC realization, introduced in Section~\ref{sec:background}, in more detail.
Specifically we say that $\F_1 \xrightarrow{\pi} \F_2$ holds when we have indistinguishability, in the standard UC sense, of the two worlds given as:

\[ (\idealP, \F_2) \sim (\pi, \F_1) \]
where \idealP is a trivial protocol such that 
\begin{mathpar}
\inferrule
{ }
{ \F \xrightarrow{\idealP} \F }
\end{mathpar}
We define realization precisely as:
