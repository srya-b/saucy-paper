...
Our sandboxing mechanism and \inline{withdrawTokens} procedure let us simulate the entire UC execution within one ITM.
We need only provide a suitable ``exchange rate function'' \GlobalF and a bounding polynomial $T$ on the single machine.
If the initial import given to the system is $n$ then the single ITM generate $n$ virtual tokens, and simulates all inputs generated by the environment.
...
It is clear to see that such a poynomial will bound the entire UC execution, and our \inline{withdrawTokens} rules ensures that we are left with a valid token context.
Therefore, the resulting ITM and the entire UC execution are also PPT in $k$.
