...
%The first composition theorem of interest replaces a single instance of a functionality with a single instance of a realizing protocol. 
%It is introduced in Section~\ref{sec:background} and we restate it here. 
...
The remainder of the main processes of \inline{execUC} are typed very similarly: accept channels relevant to them, accept all the other same paramters, and don't offer a channel.
...
Here we introduce an authenticated channel ideal functionaity, \Fauth, which exhibits this pattern and show
...
%\anote{This introduces \Fauth too out of the blue, it's a whole new example so it needs a clearer introduction, i.e. what is the functionality and what are the embodied security properties}
The communication pattern in \Fauth exhibits the same problems with the communication in Figure~\ref{newpandq}.

We therefore, are conftronted with two options: transition to a functionality where all parties and the adverasry poll for new messages or split communication among two uni-directional providerless channels.
...
The environment must communicate with multiple parties and needs multipe channels to do so. 
Furthermore, \Z having to handle creating new providerless channels to new parties is an inefficient design.
If we simplify to a single channel and multiplex, the statically-typed nature of the session type means ony a static number of parties can be specified. 
...
%The examples provided in this work do not directly use protocols that allow for an arbitrary number of parties, so we give an environment in Appendix~\ref{app:arbparties} that runs an execution with the random oracle \Fro presented in the next section.
...
\begin{figure}
	...
	\caption{This is the party wrapper that multiplexes input from other processes. It creates an instance of a protocol party (the dotted box) and sends it the message received. The party $\pi_{pid}$ is shell code akin to Figure~\ref{fig:newpandq}.\snote{Updated caption to say it's multiplxer with one instance. The reason I don't want to include another instance is that I couldn't figure out a way to add it and expand the daigram horizontally. It will leave so much white space on the sides if I add a new party and will take up much more vertical space which weneed to cut down on anyway.}}% An instance of a single protocol party.\anote{confusing since this is a single instance, yet it has a ``multiplex'' subcomponent.''} The party $\pi_{pid}$ is the shell code around the actual protocol akin to Figure~\ref{fig:newpandq}, and it is given communicators for communication in both directions. The dotted box encapsulates all the code that the \partywrapper creates for each party.}
...
Second, adding $k$ to the poynomial makes our definitions of import requirements for functionalities, like \Fro, more intuitive by requiring only 1 unit of import for one-time poynomial computations.
...
By the \m{compose} rule, the Preservation Theorem~\ref{thm:preservatin}, and Theorem~\ref{thm:global_ppt} the work done by the resulting configuration is strictly the sum of the work done by the composed configurations, and the resulting configuration \m{UC} must also be PPT with some polynmial $T(n,k)$.
