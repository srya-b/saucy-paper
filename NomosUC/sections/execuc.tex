...
\end{lstlisting}
In this example, a virtual token type, \inline{K1}, is used because of the \emph{protocol wrapper} and the \emph{functionality wrapper}.
...
$\tb{stype}$ EtoZ[a] = +{init: a ^ list[pid] -> exec} 
$\tb{stype}$ exec = &{start : output_bit} ;
$\tb{stype}$ output_bit = +{bit: Bit -> 1} ;
...
\begin{mathpar}
\mi{stype} \; \m{party}[a] = \echoice{\mb{hash} : \m{pid} \arrow \m{int} \product \m{hashing}[a]} \\
...
\begin{figure}
\centering
\includegraphics[scale=0.5]{figures/replacement.pdf}
\caption{When we compose protocols we replace the \emph{p2f} process of $\rho$ with the corresponding instance of $\pi$ and its \emph{p2f} processes. By emulation and the construction of the channels in the functionality and protocol wrappers, we are guaranteed that the type, along with the client/provider relationship, between $\rho$ and $\pi$ will type-check.}
...
\begin{proof}
The pre-condition ensures the existence of a \textit{well-resource-typed} simulator $\Sim_\pi$ for $\F_1 \xrightarrow{\pi} \F_2$, and, it is obvious that the composed protocol is also well-resource-typed.
We construct a simulator \Sim' to show
\[
    \msf{execUC}\ (\rho \circ \pi)\ \F_1 \Z \A \approx \msf{execUC}\ \rho\ \F_2 \Z \Sim
\]
and then conclude that there exists a simulator \Sim'' such that
\[
	\msf{execUC}\ (\rho \circ \pi)\ \F_1\ \Z\ \A \approx \msf{execUC}\ \idealP\ \F_3\ \Z\ \Sim''
\]	

The simulator $\Sim'$ relies only on simulating $\Sim_\pi$ for:
\[
	\msf{execUC}\ (\rho \circ \pi)\ \F_1\ \Z\ \A \approx \msf{execUC}\ \idealP\ \F_3\ \Z\ \Sim
\]	
It is a trivial construction where inputs intended for parties of $\rho$ are forwarded to them, and the inputs for parties of $\pi$ or the ideal functionalirt $\F_1$ are forwarded to $\Sim_\pi$.
Similarly, outputs from these parties are passed to $\Sim_\pi$. 
By the $\F_1 \xrightarrow{\pi} \F_2$ assumption it is clear that \Sim' satisfied
\[
    \msf{execUC} (\rho \circ \pi) \F_1 \Z \DA \approx \msf{execUC}\ \rho \F_2\ \Z \Sim'
\]

Given the above emulation proof, the fact that $\F_2 \xrightarrow{\rho} \F_3$, and the transitivity of emulation, it is clear that a simulator exists such that $\F_1 \xrightarrow{\rho \circ \pi} \F_3$.

%The constructed simulator is trivial as we pass inputs intended for parties of the protocol $\pi$ to $\SIM{\pi}$ and forward other inputs for parties of $\rho$ directly to them.
%Similary, $\SIM{\pi}$ handles outputs from $\F$
%
...
It is clear that \Sim' is \emph{well-resource-typed} as it performs constant overhead on simulating a well-resource-typd $\Sim_\pi$.
