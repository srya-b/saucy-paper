...
Here we introduce an authenticated 2-way channel ideal functionality, \Fauth, which exhibits this pattern and show
...
\parbox{0cm}{
\begin{tabbing}
$\m{sending}[a] = \textcolor{red}{\getpot^n} \ichoice{\mb{sendmsg}: \m{a} \product \m{sendmsg}}$ \\
$\m{receive}[a] = \textcolor{red}{\getpot^m} \echoice{\mb{recv}: \m{a} \product \m{receive}}$ \\
$\m{adv}[a] = \echoice{\mb{leakmsg}: a \product \ichoice{ \mb{ok}: \m{leakmsg}}}$
\end{tabbing}}
\end{center}
...
We define the output of \inline{execUC} as an ensemble of distributions over all possible random bitstrings and security parameters.
