...
The functionalities are multilexes by an additional parameter called the \msf{ssid}.
It accepts messages of the form $(\msf{ssid}, \msf{msg})$ and forwards it to the functionality instances with $\msf{sid} = \msf{ssid}$.
Unlike the party operator, the multisession operator does not use lists to manage the session types of the functionality.
Like a single functionality, it deals only with functional message types.
Only the protocol parties uses session types.
