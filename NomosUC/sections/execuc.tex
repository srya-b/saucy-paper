...
It ensures that a single ITM's execution is upper-bounded by some value $T(n)$ where $T$ is a poynomial and $n$ is the its net import \emph{token} balance (received - sent).
...
The locally PPT definition that our type system ensures that if all the individual terms in the UC experiment defined above are locally PPT then the whole execution must be as well.
To see this we reason that an ITM internally sandboxing the UC execution with virtual tokens is judged locally PPT by our type system. 
Given some configuration of ITMs in the execution, let $T'$ be the largest bounding polynomial among all ITMs, and $n$ by the initial import into the system.
It is clear to see that an ITM creating $n$ virtual tokens can simulate the execution with a bounding polynomial of $T' \times O(n)$ ($O(n)$ to facilitate handling messages and routing).
Such an ITM satisfies token context validity as we create exactly $n$ virtual tokens, and given the assumption of each simulated ITM being locally PPT, we can conclude that the entire UC execution is also PPT.
...
\footnotesize
	\inferrule*[right=emulate]
	{
		...
		\forall \A \, | \, \Delta_4[\Tokentypes][\mathrm{T}_{\A}] \A :: \Delta_4', \langle \A \leftrightarrow \pi \rangle\\
