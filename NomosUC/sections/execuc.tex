...
\begin{figure}
	...
	\caption{An instance of a single protocl party. The party $\pi_{pid}$ is the shell code around the actual protocol akin to Figure~\ref{fig:newpandq}, and it is given communicators for communication in both directions.}
...
We present a generic composition operator that works for any protocol, and a more specific, but cleaner, that works for only protocols where parties have one channel to the functionality, like \Fcom.
The reason we present two is that many functionalities can be expressed via a single session type, and simplifying composition among these processes leads to more intuitive replacement.

The basic composition operator replicated communication in Figure~\ref{fig:newpandq} and connects parties of \RHO and parties of \PI be means of a communicator.
...
When we compose two protocols we retain all of these processes and re

\begin{figure}
	\centering
	\includegraphics[scale=0.5]{figures/generalcomposition.pdf}
	\caption{Composing a party of $\rho$ with a party of $\pi$ maintains the respective protocols' intermediate processes and connects them with a communicator in the way of Figure~\ref{fig:newpandq}.}
