...
In this work we depart slightly from this definition and, instead, define the upper bound in with $T(n,k)$, a bivariate polynomial in both the net import and the security parameter $k$.
We do this for two reasons. First, it remedies our exclusion of the "parameterized systems" from the UC framework which require that ITMs don't start until they receive at least $k$ import.
Second, we add $k$ to reflect a more natural notion of import as a ``budget'' by allowing minimal import for constant-work processes and zero import for constant-work one-shot processes like \Fcom.
