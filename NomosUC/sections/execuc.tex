...
Virtualization and sandboxing\anote{are these two separate things?} of code is a common design pattern in UC, especially with adversaries and simulators.
...
Therefore, we are unable to pass in a variable number of virtual token types to the \inline{execUC}, and must rely on simple code generation\anote{what is simple code generation} to declare \inline{execUC} with a sufficient number of type parameters.
...
NomosUC departs\anote{why is this a departure? could just omit that} slightly from the standard UC framework by using a \partywrapper to encapsulate all protocol parties and run them internally.
