In this section we describe how the UC experiment is defined in Nomos and the UC notion of composition in Nomos. 
...
The UC experiment is parameterized by a set of token types, messages types, and import token types that are defined by the user and passed into the spawned processes. 
\inline{execUC} is not statically defined in Nomos. Instead, it is generated according to a protocol spcecification provided by the user.
...
For the commitment example we've used throughout this work, the \inline{execUC} process definition looks like this:
...
Therefore, \inline{execUC} definition doesn't include process definitions as parameters and refers to them through a modue \inline{PS}.

The module \inline{PS} is required to implement process \inline{PS.env} for \Z, \inline{PS.func} for the functionality, \inline{PS.prot} for the protocol, and \inline{PS.adv} for the adversary.
The environment \inline{PS.env} must have the following process definition:
...
The type parameters for all the ITMs are not the same as the type parameters passed into \inline{execUC}, though. 
...
\end{lstlisting}
For example, the channel from \Z to the protocol wrapper will typed as: \inline{comm[z2pmsg[z2p]]} where \inline{z2p} is a type parameter to \inline{execUC}.
...
...
\end{lstlisting}
Next, the environment \msf{PS.env} is spawned, and its session type requires it to send the session id, or \msf{sid}, and the list of corrupt parties to \inline{execUC}.
The rest of the ITMs are then spawned with \msf{sid} and the corrupt list.

Finally, the environment executes its own code when activated by \msf{\$z.start} and returns a bit through \inline{$\$$z.output_bit} which is its guess whether it's in the real or ideal world.
...
When \inline{execUC} spawns the ``protocol'' it actually spawns the protocol wrapper.
...
For commitment the lists for the \msf{z2p} channels are the following (\inline{R1L1} translates to ``role 1 list 1 (state 1)'':
\begin{gather}
	\m{R1L1}[\m{sender}] \\
	\m{R1L2}[\m{scommitted}] \\
	\m{R2L1}[\m{receiver}] \\
	\m{R2L2}[\m{rcommitted}] 
\end{gather}
At the start of the commitment UC execution, the \msf{z2p} channel for the committer in \inline{R1L1} indicated that it has yet to begin the commitment protocol.
Similarly if the receiver's channel was in \inline{R2L2} it is has received the commitment but not the opening.
The protocol wrappers external facing channels to \A, \F, and \Z are still functionallity typed. 
The \inline{ptof} channel in the ideal world would be typed \inline{#ptof: comm[p2fmsg[comp2f]]} for \Fcom.
...
For the commitment example, at the beginning the type of the committer's \inline{p2f} channel is given by (1). 
...
Like the protocol wrapper code, the functionality wrapper searches for the channel of the sending \inline{PID} and attempts to send the message accross it.
