...
\anote{explain in simple terms the two security properties}.

This protocol \anote{avoid overloading ``protocol''} of interaction between the sender $S$ and $\Fcom$ (and also the receiver $R$
...
\anote{The sesison type captures the communication, but not the content. In our example, the session type does not capture what, if any, information is leaked to the adversary, nor whether the bit opened is the same as the bit committed.}
...
The process is named as $\Fcom$, \emph{uses} channels $S$ and $R$ and \emph{provides} channel \inline{fc}.
