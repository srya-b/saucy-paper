...
This pattern \snote{pattern instead of protocol} of communication between the sender $S$ and $\Fcom$ (and also the receiver $R$
...
It also can does not directly capture either of \Fcom's security properties: the type can not enforce the same bit that was committed is the one that is opened.\snote{merged clarifiation on session types into this paragraph}
