...
Session types have been employed in \anote{is there a citation here? maybe of ankush's smart contract work?}, however there are several reasons they are not an obvious fit.
...
However, most prior UC formalization efforts like EasyUC and ILC forego the polynomial runtime analysis entirely, while others like IPDL and \anote{Barbosa?} achieve can enforce polynomial runtime but at the expense of restricting the UC framework to a static number of protocols.
...
\anote{finish explanation of the technical challenges in making session types while still supporting the dynamic nature of UC.}
...
\anote{explain why sandboxing requires us to introduce virtual tokens}.
