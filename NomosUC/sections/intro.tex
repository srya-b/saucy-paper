...
\begin{enumerate}
\item Increasingly complex cryptographic applications often involve several parties communicating according to a pre-defined protocol, and implementations of them can deviate from their intended behavior.
However, existing tools often do not provide a streamlined approach of expressing and enforcing
such protocols.
Can the programming language alert its user to such deviations in their code?
...
We show that for some functionalities, like \Fflip used in Section~\ref{sec:commitment}, we are able to use polymorphic types to extend 
the power of the type to enforce ordering between messages in different channels as well.
