...
tokens of each type contained by the process and the globally known \emph{security parameter} $k$.
...
\begin{mathpar}
  ...
  {\Tokens\;\; \m{valid} \and
  K_{i} \hookrightarrow (t_i, t_i') \in \Tokens \and k \in \Tokens \and
...
that $t_{i+1} + n \leq \GlobalF(t_i',k)$ where $K_i \hookrightarrow (t_i, t_i') \in \Tokens$ and $k \in \Tokens$.
