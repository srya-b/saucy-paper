...
$\m{withdrawToken} \; f \; K' \; (m : K)$.
\begin{mathpar}
  \inferrule*[right=$\m{tok}$]
  {f(m) \leq n \\\\ \Tokens, K \hookrightarrow n, K' \hookrightarrow f(m) \semi
  \Psi \semi\wt, \D \entailpot{q} P :: (x : A)}
  {\Tokens, K \hookrightarrow n \semi \Psi \semi \wt, \D \entailpot{q} \hspace{8em} \\
    \hspace{5em}\m{withdrawToken} \; f \; K' \; (m : K) \semi P :: (x : A)}
\end{mathpar}
