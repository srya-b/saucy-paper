...
The pattern emerges from the following code: a machine $M$ either writes to another machine $N$ or is written to by $N$. 
At first glance, it is a trivial scenario, but we encounter a problem trying to encode this with a single session type.
A real example of this scenario is give by three processes $P$, $Q_1$, and $Q_2$ which execute as follows:
\begin{itemize}
	\item $P$ flips a bit and writes to either $Q_1$ or to $Q_2$
	\item If $Q_1$ is activated by $P$ it writes to $Q_2$ and the execution terminates. 
	\item Otherwise, if $Q_2$ is activated by $P$, it writes a message to $Q_1$, $Q_2$ writes something back, and the execution terminates.
\end{itemize}
A single type between $Q_1$ and $Q_2$ would have to allow either of the two parties to write on the channel, but session types require it to be statically known.

If we try to separate communication between $Q_1$ and $Q_2$ into two uni-directional channels, we end up with Figure~\ref{fig:pandq}, and can express the session type for each of the channels:
\begin{center}
\parbox{0cm}{
\begin{tabbing}
$\m{q1toq1} = \ichoice{\mb{``one''}: int \tensor 1}$ \\
$\m{q2toq1} = \echoice{\mb{``one''}: int \tensor \ichoice{\mb{``end''}: int \tensor 1}}$
\end{tabbing}}
\end{center}
...
Without adding any additional processes, $Q_1$ must offer a channel to $Q_2$ and $Q_2$ offers one to $Q_1$. 
...
We wrap each of $Q_1$ and $Q_2$ in shell code, call them $S_1$ and $S_2$, which spawn two dummy proceses to offer each of the channels of type \m{q1toq2} and \m{q2toq1} to $Q_1$ and $Q_2$.
...
\begin{figure}
	...
	\caption{Implementing the configuration with two channels is also impossible because $Q_1$ can't be both client and provider for $Q_2$, and vice versa.}
	\label{fig:pandq}
	\end{subfigure}
	~ \ \ \ \ 
	...
	\caption{We can systematically convert any such execution into one intermediated by communicators in a mechanical way. Session types are still used by $P$ and $Q$ but a cycle is avoided through the used oa communicators offering a shared channel.}
	\label{fig:newpandq}
	\end{subfigure}
	\caption{An ITM configuration realized in NomosUC.}
\end{figure}
...
Returning to the initial example of $Q_1$ and $Q_2$, although the contruction appears excessive, it is generic and can intermediate communication between any pair of processes in NomosUC. 
