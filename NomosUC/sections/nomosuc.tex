...
\begin{mathpar}
  ...
  \mi{stype} \; \m{scommitted} = \ichoice{\mb{open} : \m{sopen}} \\
  \mi{stype} \; \m{sopen} = 1
\end{mathpar}
...
\begin{mathpar}
	...
	\mi{stype} \; \m{rcommitted} = \echoice{\mb{open} : \m{bit} \arrow \m{ropen}} \\
	\mi{stype} \; \m{ropen} = 1
\end{mathpar}
...
of channels \emph{used} by $\Fcom$, while $F$ is the channel \emph{provided}
by $\Fcom$.
...
\todo{Is closing strictly necessary in NomosUC}

A session type uniquely defines the protocol for only one role in an ideal functionality.
The sender and receiver do different things, therefore, must have their own channels 
to \Fcom rather than communicating over a common channel.  
...
$\Tokens \semi \Psi \semi \D \entailpot{q}{q'} P :: (x : A)$.
...
tokens of each type contained by the process.
...
\begin{mathpar}
  \footnotesize
  \infer[{\one}R]
  {\B{\Tokens \semi \Psi} \semi \wt \entailpot{\B{q}}{\B{q'}} \eclose{x} :: (x : \one)}
  {\B{q = 0}}
  \and
  \infer[{\one}L]
  {\B{\Tokens \semi \Psi} \semi \D, (x : \one) \entailpot{\B{q}}{\B{q'}} (\ewait{x} \semi Q) :: (z : C)}
  {\B{\Tokens \semi \Psi} \semi \wt, \D \entailpot{\B{q}}{\B{q'}} Q :: (z : C)}
\end{mathpar}
...
$K_i$, it can only contain at most $\GlobalF(m)$ total tokens of type
...
\begin{mathpar}
  ...
  \and
  \infer
  {\Tokens, K_{i+1} \hookrightarrow (t_{i+1}, t_{i+1}')\;\; \m{valid}}
  {\Tokens\;\; \m{valid} \and
  K_{i} \hookrightarrow (t_i, t_i') \in \Tokens \and
  t_{i+1} \leq \GlobalF(t_i')}
\end{mathpar}
...
that $t_{i+1} + n \leq \GlobalF(t_i')$ where $K_i \hookrightarrow (t_i, t_i') \in \Tokens$.
...
the continuation.
In this case, the validity of the token context establishes that $t_{i+1} \leq \GlobalF(t_i')$,
...
\begin{mathpar}
  ...
  {q+r \leq \GlobalF(t_{\depth}') \and K_{\depth} \hookrightarrow (t_{\depth}, t_{\depth}') \in \Tokens \\\\
  \Tokens \semi \Psi \semi \wt, \D \entailpot{q+r}{q'+r} P :: (x : A)}
  {\Tokens \semi \Psi \semi \wt, \D \entailpot{q}{q'} \m{genPot} \; r \semi P :: (x : A)}
\end{mathpar}
...
is bounded by $\GlobalF(t_{\depth}')$ where $(t_{\depth}, t_{\depth}')$ is the number of tokens
